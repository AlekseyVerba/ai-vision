...
export class UserService {
    ...
    async updateUser({avatars, ...dto}: UpdateUserDto) {
        if (avatars) {
            await this.userAvatarsRepository.update({...avatars, default: avatars.defaultImage}, {where: {user_uid: dto.uid}})
        }

        return (await this.userRepository.update(dto, {where: { uid: dto.uid }, returning: true}))[1][0]
    }
