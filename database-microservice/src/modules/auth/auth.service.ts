...
export class AuthService {
    ...
    async resetPassword(email: string) {
        ...
        this.clientMailer.emit('message.reset-password', {name: user.name, email: user.email, token: value })

        return 'A message with token has been sent to your email!'
    }

    async login({ email, password }: LoginDto) {
        let user = await this.userRepository.scope({password}).findOne({where: {email}})
    ...
    async registration(dto: RegistrationDto) {
        ...
        try {
            ...
            this.userAvatarsRepository.create({user_uid: newUser.uid}, { transaction })
            ...
            this.clientMailer.emit('message.registration', {name: dto.name, email: dto.email, token: value })
    ...
    async confirmNewPassword({ token, password }: ConfirmNewPasswordDto) {
        ...
        try {
            ...
            await this.userRepository.update({password: newHashPassword}, {where: {uid: currentToken.user_uid}})
    ...
    createJwtToken(user: User) {
        return sign({ uid: user.uid, email: user.email, name: user.name, is_active: user.is_active }, process.env.JWT_SECRET || '1111', {
            expiresIn: '3h'
        })
    }
}