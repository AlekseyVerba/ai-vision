...
export class UserService {
    ...
    async updateUser({ avatar, ...dto }: UpdateUserInput) {
        ...
        if (avatar) {
            ...
            const userAvatars = await lastValueFrom(this.client.send('get-user-avatars', dto.uid))
            ...
            const [defaultImage, small, middle, large] = await Promise.all([
                this.fileService.uploadFile({ file: { buff, filename: normalAvatar.filename }, dir: `user/${dto.uid}` }),
                this.fileService.createResizedImage({buff, filename: normalAvatar.filename}, `user/${dto.uid}`, 70, {
                    height: 33,width: 33, options: { fit: 'contain' }
                }),
                this.fileService.createResizedImage({buff, filename: normalAvatar.filename}, `user/${dto.uid}`, 70, {
                    height: 66,width: 66, options: { fit: 'contain' }
                }),
                this.fileService.createResizedImage({buff, filename: normalAvatar.filename}, `user/${dto.uid}`, 70, {
                    height: 128,width: 128, options: { fit: 'contain' }
                })
            ])
